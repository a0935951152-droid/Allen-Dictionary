...
| 4 | `POST /api/ops/judge`（推薦）｜`POST /api/batches/{b}/coarse_fix`（窗口開著時單批直判） | **判官（Gemma 獨佔窗口）**：吃兩軸證據＋整段接地錨點＋平行多語 → `{abandon,xling}` → 四態（①修 ②維持 ③人工 ④—）。ops 走主機代理：清 GPU→載 Gemma→`/api/judge_pending` 批次判所有存疑→換回；已判 human 的 span 不重擲骰。**常用原詞永不 auto（封頂人工）** |
...
| 端點 | 作用 |
|---|---|
| `POST /api/ops/build` | **前端一鍵建庫（推薦）**：排入 ops 佇列 → 主機 `dic agent` 開 Gemma 獨佔窗口執行 → `GET /api/ops/status` 輪詢進度（前端進度條）。body `{clear:bool}` |
| `POST /api/knowledge/build` | sources → 兩階段 Gemma 抽取 → vault+CAG+acoustic（增量；`clear`重整）。**需 Gemma 窗口已開**（ops 版免操心） |
...
`POST /api/ops/{build|judge}`＋`GET /api/ops/status`（獨佔窗口佇列）；`GET /api/pipeline/params`（參數視覺化，唯讀）；
...
| 判官併發 | `JUDGE_CONCURRENCY` | 4 | Gemma 窗口內 vLLM 併發 |
